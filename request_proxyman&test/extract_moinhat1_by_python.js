...
const logFile = '/Users/hoangquan/Downloads/moinhat1.proxymanlogv2';
const tempDir = '/Users/hoangquan/Downloads/temp_moinhat1_python';
...
console.log("Đang giải nén moinhat1.proxymanlogv2 (132MB) bằng Python...");
try {
    execSync(`python3 -c "import zipfile; zipfile.ZipFile('${logFile}').extractall('${tempDir}')"`);
    console.log("Giải nén moinhat1 thành công!");
} catch(e) {
...
console.log(`Tổng số file giải nén được từ moinhat1: ${allFiles.length}`);

let found = 0;
allFiles.forEach(filePath => {
    try {
        const content = fs.readFileSync(filePath, 'utf-8');
        if (content.toLowerCase().includes('user') && content.toLowerCase().includes('init')) {
            ...
            if (url.toLowerCase().includes('user/init')) {
                found++;
                console.log(`\n=========================================`);
                console.log(`[ĐÃ TÌM THẤY TRONG MOINHAT1] File: ${path.relative(tempDir, filePath)}`);
                console.log(`URL: ${url}`);
...
console.log(`Quét xong! Tìm thấy ${found} request init.`);
execSync(`rm -rf "${tempDir}"`);
