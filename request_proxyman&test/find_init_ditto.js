...
const tempDir = '/Users/hoangquan/Downloads/temp_ditto';
...
logFiles.forEach(zipFile => {
    ...
    console.log(`Đang giải nén bằng ditto: ${path.basename(zipFile)}...`);
    try {
        // Dùng ditto giải nén cực kỳ an toàn và ổn định
    ...
    try {
        ...
        if (content.toLowerCase().includes('user') && content.toLowerCase().includes('init')) {
            ...
            if (url.toLowerCase().includes('user/init')) {
                found++;
                console.log(`\n=========================================`);
                console.log(`[TÌM THẤY] File: ${path.relative(tempDir, filePath)}`);
                console.log(`URL: ${url}`);
                ...
                console.log(`\nResponse Body (Chỉ in 500 ký tự):`);
                if (res.bodyData) {
                    ...
                    console.log(dec.toString('utf-8').slice(0, 500));
                } else {
...
console.log(`\nTổng số request init thực sự tìm thấy: ${found}`);
execSync(`rm -rf "${tempDir}"`);
